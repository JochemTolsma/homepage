...
$$VAR(X)=s_{xx}^2 = s_x^2= \frac{\Sigma^n_i(X_i-\overline{X})(X_i-\overline{X})}{n-1}= \frac{\Sigma^n_i(X_i-\overline{X})^2}{n-1}$$  
(And the standard deviation is given by: $STD(X)=s_x$.)  

The sample covariance of two random continuous variables X and Y, Cov(X,Y) is as follows:  

$$COV(X,Y)=s_{xy}^2 = \frac{\Sigma^n_i(X_i-\overline{X})(Y_i-\overline{Y})}{n-1}$$
The correlation is a covariance on standardized variables ($z_x=X_{sd}=(X-\overline{X})/s_x$) and hence:  

$$CORR(X,Y)=r_{xy} = \frac{s_{xy}^2}{s_x s_y}= \frac{\Sigma^n_i(X_i-\overline{X})(Y_i-\overline{Y})}{\sqrt{\Sigma^n_i(X_i-\overline{X})}\sqrt{\Sigma^n_i(Y_i-\overline{Y})}}$$
