...
G2 <- graph_from_data_frame(edges, directed=TRUE, vertices=keyf)
plot(G2)

```
Mmm, It looks like MPs do like to mention themselves! 
...
plot(G2_sel)
```
And why do we still see isolates??!! ... 

Dont know. After havig inspect the degrees etc. There are no isolates. 
...
```
```{r}
#bit clumsy but I want my data back
keyf <- twitter_20190919[[1]]
...
plot.igraph(G2, layout=owncoords, rescale=F, margin=0)
