...
{{% alert warning %}}  
Assignment 7 is way too difficult. First, in Lavaan it is not straightforward to estimate interaction effects when the interaction variable involves a latent-variable. Second, the model will not run if you include size in a model with the two dependent variables simultaneously. 
{{% / alert  %}}  
...
Download data [here](static/sn2021_egonetdata_v1.Rdata).   
...
## two dependent variables, two micro-level indicators

```{r ,  attr.source = '.numberLines', results='hold'}

#two individual-level predictor, one latent variable at group level
model2 <- '
  #latent variable
  FX =~ 1*alt1_xa
  FX =~ 1*alt2_xa
...
fit1 <- lavaan(model1, data = data_wide, missing = "fiml", fixed.x=FALSE)
summary(fit1)
