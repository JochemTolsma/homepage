...
categories:
  ...
  - Social Networks
tags: []
linktitle: Consequences of Dyads (methods)
...
{{% alert warning %}}
In the upper left corner of the code blocks you will find a tiny copy-to-clipboard button. Use this button to copy the code to your own editor. I don't know why this button is so small on the website. I will try to fix this issue asap.
{{% /alert %}}
...
Don't forget, always start with cleaning up your workspace. 
...
Y_Wt2 <- (mean(Y_W) + 3) + .6*(Y_W - mean(Y_W)) + .4*(Y_M - mean(Y_M)) + errort2[,1]
Y_Mt2 <- (mean(Y_M) + 5) + .5*(Y_W - mean(Y_W)) + .7*(Y_M - mean(Y_M)) + errort2[,2]
...
Also check out the book by Snijders and Boskers (1999)[^1]  

[^1]: ref

Lets start with something that pops up in your mind immediately...a correlation and for those of you with some experience in multilevel analysis...intraclass correlation. 
...
Most of you are probably more familiar with definitions of the ICC as provided within textbooks on multi-level analysis. Where the intraclass variance - at least for continuous dependent variables - is defined as the between variance (i.e. the variance in dyad means) divided by the total variance (i.e. the sum of the between and within variance). There is only one problem, we need these variances present in the 'real population'. In our data we only observe the variances present in our sample. The observed between variance needs to be corrected. Below I will show you have to do that. 
...
But this is of couse quite cumbersome. Thus in practice I personally simply calculate a correlation on the variabels of interest in a dataset in long format. Or, alternatively, estimate an empty multi-level model. Which also spits out the ICC (after some tweaking). 
See below. 
...
Perhaps, you would have realized that the impact of own and partner's age and educ may depend on your sex. Thus you may have estimated the model separately for men and women. There would be nothing wrong with that approach, because with one observation per dyad/household the observations can still be regarded as independent. Thus:...
...
One approach is to estimate both models simultaneously within a SEM framework. 
...
```
So, what would you conclude now!!??
